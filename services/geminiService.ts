...
export const generateFestiveMessage = async (name: string): Promise<string> => {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: `Generate a unique, short, heartwarming Christmas 2026 message for ${name}. 
    ...
    return response.text?.trim() || `Merry Christmas 2026, ${name}! May your days be filled with unique magic and unexpected joy.`;
  } catch (error) {
    console.error("Error generating message:", error);
    // Fallback messages with randomness
    const fallbacks = [
      `Merry Christmas 2026, ${name}! Wishing you a year as brilliant as the winter stars.`,
      `To ${name}: May the magic of this season wrap you in warmth and wonder.`,
      `Happy 2026 Holidays, ${name}! May your heart be light and your home be full of laughter.`,
